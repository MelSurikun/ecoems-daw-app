...
Usage:
  ...
  python3 backend/etl/carga_planteles.py temp/OPC_EDU_2025.pdf --solo=UNAM,IPN
  → outputs planteles_inserts.sql in temp/

Parses pdftotext -layout output. Deduplicates by CLAVE.
--solo=SUB1,SUB2  restricts the output to those `subsistema` values only
                  (matched after normalization via INST_MAP).
"""
import re, sys, subprocess, os

args = [a for a in sys.argv[1:] if not a.startswith('--')]
solo_arg = next((a for a in sys.argv[1:] if a.startswith('--solo=')), None)
SOLO = set(s.strip().upper() for s in solo_arg.split('=', 1)[1].split(',')) if solo_arg else None

pdf_path = args[0] if args else 'temp/OPC_EDU_2025.pdf'
...
for i in range(len(lines)):
    ...
    if m:
        ...
        # FALLBACK ONLY: el centroide de alcaldía no es la ubicación real del
        # plantel (puede caer en un parque, una avenida, etc.). Úsalo solo
        # como aproximación gruesa para revisar manualmente; no lo tomes
        # como coordenada definitiva para producción (ver database/planteles_cdmx.sql,
        # que sí tiene coordenadas reales verificadas por campus).
...
# Optional subsistema filter (e.g. --solo=UNAM,IPN)
if SOLO is not None:
    records = {k: v for k, v in records.items() if v[3].upper() in SOLO}
...
with open(out_path, 'w', encoding='utf-8') as f:
    ...
    if SOLO is not None:
        f.write(f"-- Filtered to subsistema in: {', '.join(sorted(SOLO))}\n")
